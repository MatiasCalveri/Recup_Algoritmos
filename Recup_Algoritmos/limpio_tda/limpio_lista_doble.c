...
///version nueva de poner en orden
int poner_ord_lista(tListaDoble* pl, const void* pd,
                    size_t tam, int (*cmp)(const void*, const void*))
{
    tNodo* act = *pl;
    tNodo* ant = act ? act->nAnt : NULL;
    tNodo* nue;
    ...
    if( (act && cmp(pd, act->info) == 0) ||
        (ant && cmp(pd, ant->info) == 0) )
        return -1; /// DUPLICADO

    /* --- CREAR NODO --- */
    nue = (tNodo*)malloc(sizeof(tNodo) + tam);
    if(!nue)
        return -2; /// FALTA DE MEMORIA
...
///version vieja de poner en orden
